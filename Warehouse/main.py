    ...
    if (check_substring(text, 'insert') or check_substring(text, 'insect')):
        ...
        if (len(auxText) > 1):
            ...
            if (check_substring(text, 'units')):
                print(f"Adding: {what_to_add}")
                add_to_list(what_to_add, detect_ammount(auxText[1]))
        ...
        if confirmation == 'yes' or confirmation == 'confirm' or confirmation == 'correct':
            print("Saving shopping list...")
            shopping_list = [[line.strip()] for line in lines]
            save_csv(shopping_list, 'shopping_list.csv')
            not_confirmed = False
        else:
